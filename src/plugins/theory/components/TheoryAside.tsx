/**
 * [Заморожено 2026-09-02] TheoryAside — левая боковая панель theory-viewer:
 *   • «Структура» — интерактивный outline документа из заголовков (данные готовит lib/outline.ts);
 *   • «Готовность» — карточка прогресса (заглушка макета, реальной метрики нет);
 *   • «Материалы» и «Теги» — статичные заглушки (бэкенда для них пока нет).
 *
 * Панель временно убрана из UI по решению пользователя; компоненты сохранены
 * на будущее. Файл закомментирован целиком.
 * Чтобы вернуть панель:
 *   1) раскомментировать этот файл и парный TheoryAside.style.ts;
 *   2) в viewer.tsx раскомментировать блок «Aside (outline)» и рендер <TheoryAside>.
 */
