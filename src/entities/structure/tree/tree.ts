 ...
 *
 * ## Роль
 * Вычислительное ядро иерархии узлов структуры. Любое
 ...
 * - Держать его в jotai-атоме как неизменяемое значение
 *
 * ## Устройство
...
export class Tree {
   ...
   *
   * Используется стором сущности для производной плоской проекции
   * (structureNodesAtom) и для конвертации во вложенную модель рендера.
   *
   ...
   * поддерева (см. RemoveAction в store/actions.ts).
   *
