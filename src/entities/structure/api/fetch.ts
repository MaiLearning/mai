...
export function fetchStructure(courseId: string): Promise<StructureNodeFlat[]> {
  return isFakeDataEnabled ? Promise.resolve(fakeState.nodes.filter((node) => node.courseId === courseId).map((node) => ({ ...node }))) : invoke<StructureNodeFlat[]>('get_structure', { courseId })
}
